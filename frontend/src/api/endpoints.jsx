...
const cleanApi = axios.create({
    baseURL: BASE_URL,
    //kg   NO withCredentials (hindi magse-send ng cookies)
    timeout: 5000
})
...
export const register = async(username, email, firstName, lastName, password) => {
    const response = await cleanApi.post(`/register/`, {username:username, email:email, first_name:firstName, last_name:lastName, password:password})
    return response.data
}
